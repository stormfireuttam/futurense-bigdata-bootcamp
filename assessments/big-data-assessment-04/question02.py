...
sc = SparkContext(appName="MovieStream")

# Define the schema for the movie.csv dataset
...
movies_stream = ssc \
    .readStream \
    .format("csv") \
    .option("path", "/user/training/movie/movies1.csv") \
    .load()

# Convert the CSV data to a DataFrame using the defined schema
movie_df = movies_stream\
    .select(from_csv(col("value"), movie_schema).alias("movie"))\
